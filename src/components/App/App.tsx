...
export default class App extends React.Component<AppProps, AppState> {
      ...
      const ytPlayer = new window.YT.Player('leftYt', {
        events: {
          ...
          onStateChange: (e: any) => {
            ...
            // console.log(this.ytDebounce, e.data, this.shaconYTPlayer?.getVideoUrl());
            if (
              this.ytDebounce &&
  ...
  join = async (roomId: string) => {
    ...
    try {
      const savedPasswordsString =
        window.localStorage.getItem('shacon-passwords');
    ...
    socket.on('REC:host', async (data: HostState) => {
      ...
      this.setState(
        ...
        () => {
          ...
          leftVideo?.pause();
          this.shaconYTPlayer?.stopVideo();

          if (this.isYouTube() && !this.shaconYTPlayer) {
            console.log(
  ...
  getCurrentTime = () => {
    ...
    if (this.isYouTube()) {
      return this.shaconYTPlayer?.getCurrentTime();
    }
  };
  ...
  getDuration = () => {
    ...
    if (this.isYouTube()) {
      return this.shaconYTPlayer?.getDuration();
    }
    return 0;
  };
  ...
  isMuted = () => {
    ...
    if (this.isYouTube()) {
      return this.shaconYTPlayer?.isMuted();
    }
    return false;
  };
  ...
  isSubtitled = () => {
    ...
    if (this.isYouTube()) {
      try {
        const current = this.shaconYTPlayer?.getOption('captions', 'track');
  ...
  doSrc = async (src: string, time: number) => {
    ...
    if (this.isYouTube()) {
      ...
      this.shaconYTPlayer?.cueVideoById(videoId || altVideoId, time);
    }
  };
  ...
  doPause = () => {
    this.setState({ currentMediaPaused: true }, async () => {
      ...
      if (this.isYouTube()) {
        console.log('pause');
        this.shaconYTPlayer?.pauseVideo();
      }
    });
  };
  ...
  doSeek = (time: number) => {
    ...
    if (this.isYouTube()) {
      this.shaconYTPlayer?.seekTo(time, true);
    }
  };
  ...
  togglePlay = () => {
    ...
    if (this.isVideo()) {
      ...
      shouldPlay =
        this.shaconYTPlayer?.getPlayerState() ===
  ...
  setMute = (muted: boolean) => {
    ...
    if (this.isYouTube()) {
      if (muted) {
        this.shaconYTPlayer?.mute();
      } else {
        this.shaconYTPlayer?.unMute();
      }
    }
  };
  ...
  setVolume = (volume: number) => {
    ...
    if (this.isYouTube()) {
      this.shaconYTPlayer?.setVolume(volume * 100);
    }
  };
  ...
  getVolume = () => {
    ...
    if (this.isYouTube()) {
      const volume = this.shaconYTPlayer?.getVolume();
      return volume / 100;
    }
  };
  ...
  toggleSubtitle = () => {
    ...
    if (this.isYouTube()) {
      ...
      if (isSubtitled) {
        ...
        this.shaconYTPlayer?.setOption('captions', 'track', {});
      } else {
        this.shaconYTPlayer?.setOption('captions', 'reload', true);
        const tracks = this.shaconYTPlayer?.getOption('captions', 'tracklist');
        this.shaconYTPlayer?.setOption('captions', 'track', tracks?.[0]);
      }
    }
  };
  ...
  updateName = (_e: any, data: { value: string }) => {
    ...
    window.localStorage.setItem('shacon-username', data.value);
  };
                          ...
                          {!this.state.loading &&
                            this.state.nonPlayableMedia && (
                              <Message
                                ...
                                content="Please upload a supported a file format"
                              />
                            )}
                        </div>
                      )}
