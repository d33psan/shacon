    ...
    <Modal.Content>
      <Form>
        <TextArea rows={10} id="settings_textarea">
          {window.localStorage.getItem('shacon-setting') ||
...
export function getCurrentSettings(): Settings {
  const setting = window.localStorage.getItem('shacon-setting');
  try {
...
export function updateSettings(newSetting: string) {
  window.localStorage.setItem('shacon-setting', newSetting);
}
